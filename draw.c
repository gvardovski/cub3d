...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/30 16:13:14 by svolkau           #+#    #+#             */
/*   Updated: 2025/10/30 16:13:18 by svolkau          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_line(t_data *g, int dir, int screen_x)
{
	...
	if ((g->ray.side == 0 && g->ray.dir[X] > 0) || (g->ray.side == 1 && g->ray.dir[Y] < 0))
		tex[X] = g->c.tex[dir].width - tex[X] - 1;
	tex[Y] = (g->ray.draw_start - HEIGHT / 2.0 + g->ray.line_height / 2.0) * ((double)g->c.tex[dir].height / (double)g->ray.line_height);
...
void put_texture(t_data *g, double tex[2], int dir, int x)
{
	...
	while (y <= g->ray.draw_end)
	{
		...
		color = *(unsigned int *)(g->c.tex[dir].addr + (int)tex[Y] * g->c.tex[dir].line_len + (int)tex[X] * (g->c.tex[dir].bpp / 8));
		if (g->ray.side == 1)
